...
object SInvolvementDocAttachmentInfoTransformer {

  def docAttachment(messageId: String, serviceDelivery: ServiceDelivery, timestamp: Long): List[SServiceDeliveryDocAttachmentInfo] = {
    val source = ""
    val fmt = ISODateTimeFormat.dateTime()
    val time = new DateTime(timestamp, DateTimeZone.UTC)

    serviceDelivery.involvements.sorted.flatMap(involvement => involvement.documentAttachments.sorted.map(docAttachment => {
      ...
      val doc_atchmnt_hk = HashHelper.sha1(Seq(
        ...
        ("srvc_dlvry_handle_id", serviceDelivery.internalHandle.interface_identifier),
        ("invlmnt_handle_id", involvement.internalHandle.interface_identifier),
        ("doc_atchmnt_handle_id", docAttachment.internalHandle.interface_identifier)))
  ...
  def docAttachmentStr(documentAttachment: DocumentAttachment): List[(String, String)]= {
    List(
