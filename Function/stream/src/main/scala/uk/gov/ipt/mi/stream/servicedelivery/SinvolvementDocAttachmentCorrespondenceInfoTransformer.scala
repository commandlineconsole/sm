    ...
    serviceDelivery.involvements.sorted.flatMap(involvment => involvment.documentAttachments.sorted.flatMap(docAttach =>
      docAttach.correspondences.map(correspondences => correspondences.sorted.map((corr: Correspondence) => {
        val corr_rec_hash_value = HashHelper.sha1(Seq(
          ...
          ("corr_last_updated", corr.lastUpdated.getOrElse("")),
          ("corr_status", corr.status.getOrElse(""))
        ))
        ...
        val invlmnt_hk = HashHelper.sha1(Seq(
          ("record_source",source),
          ("srvc_dlvry_handle_id", serviceDelivery.internalHandle.interface_identifier),
          ("invlmnt_handle_id", involvment.internalHandle.interface_identifier),
        ...
        SServiceDeliveryCorrInfo(messageId, DateHelper.getMostRecentDate(List(serviceDelivery.created, serviceDelivery.createdDate, serviceDelivery.lastUpdated)), fmt.print(time),
          source, serviceDelivery.internalHandle.interface_identifier, involvment.internalHandle.interface_identifier,
          ...
          srvc_dlvry_hk, invlmnt_hk, doc_atchmnt_hk, corr_hk)
      }
      )).getOrElse(List())))
  }
