...
#----------------
##
## Spark helper script to manage the application ${ipt-application.name}
##
## Spark properties are build from the generic.properties in the rpm and the local properties
## in /etc/${ipt-application.name}/local.properties
##
## This can be overridden by setting the environment variable:
##
##  * SPARK_LOCAL_PROPERTIES
##    eg: export SPARK_LOCAL_PROPERTIES=/etc/${ipt-application.name}/conf/local.properties.restore
##
##  * SPARK_GENERIC_PROPERTIES
##    eg: export SPARK_GENERIC_PROPERTIES=/opt/${ipt-application.name}/${project.version}/config/generic.properties.restore

usage ()
{
    echo "$(basename $0): IPT wrapper to start and deploy Spark applications"
    echo  -e "Usage:\n"
...
if [ -z "$SPARK_LOCAL_PROPERTIES" ]; then
    SPARK_LOCAL_PROPERTIES=/etc/${APP}/conf/local.properties
fi
...
hdfs_spark()
{
    ...
    su - hdfs -c "hdfs dfs -mkdir -p /usr/hadoop/yarn/local/usercache/spark"

    su - hdfs -c "hdfs dfs -chmod 755 /var/lib/spark"
    su - hdfs -c "hdfs dfs -chmod 755 /var/log/spark"
    su - hdfs -c "hdfs dfs -chmod 755 /usr/hadoop/yarn/local/usercache/spark"

    su - hdfs -c "hdfs dfs -chown ${APP_USER}:${APP_USER} /var/lib/spark"
    su - hdfs -c "hdfs dfs -chown ${APP_USER}:${APP_USER} /var/log/spark"
    su - hdfs -c "hdfs dfs -chown ${APP_USER}:${APP_USER} /usr/hadoop/yarn/local/usercache/spark"
...
distribute()
{
    ...
    #Creates a stream directory on HDFS if one does not exist already
    ret=$(su - hdfs -c "hdfs dfs -test -d /apps/MI/stream")
    if [ $? -ne 0 ]; then
        echo "Creating hdfs:///apps/MI/stream"
        su - hdfs -c "hdfs dfs -mkdir /apps/MI/stream"
        su - hdfs -c "hdfs dfs -chmod 755 /apps/MI/stream"
        su - hdfs -c "hdfs dfs -chown ${APP_USER}:${APP_USER} /apps/MI/stream"
    fi
    ...
    # DELETE checkpoint on redeploying
    ret=$(su - hdfs -c "hdfs dfs -test -d /apps/MI/stream/checkpoint")
    if [ $? -eq 0 ]; then
        echo "Deleting previous checkpoint directory"
        su - hdfs -c "hdfs dfs -rm -skipTrash /apps/MI/stream/checkpoint"
        su - hdfs -c "hdfs dfs -mkdir /apps/MI/stream/checkpoint"
        su - hdfs -c "hdfs dfs -chmod 755 /apps/MI/stream/checkpoint"
        su - hdfs -c "hdfs dfs -chown ${APP_USER}:${APP_USER} /apps/MI/stream/checkpoint"
    fi
    ...
    if [ $? -ne 0 ]; then
        ...
        if [ "$STREAM_NAMESPACING" != "" ]; then
            ...
            su - ${APP_USER} -c "hdfs dfs -rm -skipTrash /apps/MI/${APP}/${STREAM_NAMESPACING}/current-dist.tar.gz"
            ...
            su - ${APP_USER} -c "hdfs dfs -rm -skipTrash /apps/MI/${APP}/current-dist.tar.gz"
...
hdfs_clean()
{
    if [ "$STREAM_NAMESPACING" != "" ]; then
        ...
        su - hdfs -c "hdfs dfs -rm -skipTrash /apps/MI/${APP}/${STREAM_NAMESPACING}/${APP_PKG}"
    else
        echo "Cleaning hdfs:///apps/MI/${APP}/${APP_PKG}"
        su - hdfs -c "hdfs dfs -rm -skipTrash /apps/MI/${APP}/${APP_PKG}"
    fi
}
...
start() {
    ...
    --deploy-mode cluster \
    --files ${APP_PATH}/config/${APP}.properties.rpm \
    --executor-memory ${EXECUTOR_MEMORY} \
    --executor-cores ${EXECUTOR_CORES} \
    ${APP_PATH}/lib/${APP}-${ipt-application.version}.jar \
    -c ${APP}.properties.rpm \
